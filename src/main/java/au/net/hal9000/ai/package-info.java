...
The eventual aim of this package is to allow Agents to move rationally within the game.
...
Cost - A measure of the effort in performing the action. e.g. the distance of a step modified by gradient of ground.
...
Successor - An Action, Cost and the ModelState that action will lead to.<br>
...
SuccessorFunction - A function that produces a list of Successor objects from current ModelState. <br>
   e.g. the function might produce a list of directions that may be stepped; each with the cost (effort) of each step and the new location reached.
